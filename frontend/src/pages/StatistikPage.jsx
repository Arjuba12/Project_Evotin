...
const chartOptions = {
  ...
  scales: {
    x: { ticks: { color: "#71717a" }, grid: { color: "#2a2a30" } },
    y: { ticks: { color: "#71717a" }, grid: { color: "#2a2a30" } },
  },
};
...
export default function StatistikPage() {
  ...
  if (!stats || results.length === 0) return (
    <div className="statistik-page"><p style={{ color: "var(--text-muted)" }}>Memuat data...</p></div>
  );

  const barData = {
    labels: results.map((r) => r.name),
    datasets: [{ label: "Jumlah Suara", data: results.map((r) => r.total_votes), backgroundColor: "#6366f1", borderRadius: 6 }],
  };

  const pieData = {
    labels: results.map((r) => r.name),
    datasets: [{ data: results.map((r) => r.total_votes), backgroundColor: ["#6366f1", "#8b5cf6", "#a78bfa"], borderWidth: 0 }],
  };

  const participation = stats.total_users > 0 ? ((stats.total_votes / stats.total_users) * 100).toFixed(1) : 0;
  ...
  return (
    <div className="statistik-page">
      ...
      <div className="chart-container">
        ...
        <Pie data={pieData} options={{ plugins: { legend: { labels: { color: "#a1a1aa" } } } }} />
      </div>
    </div>
  );
}
